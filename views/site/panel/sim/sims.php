...
$gridColumns = [
    ...
    [
        'attribute' => 'iccid',
        'content' => function($model) {
           $url = Url::toRoute('site/editsim', array('id' => $model->id));
           return "<a id=\"{$model->id}\" onclick=\"event.preventDefault();simEdit(this);return false;\" href=\"$url\">$model->iccid</a>";
        }
            ],
            'msisdn',
            'preload_value',
            ...
            [
                ...
                'content' => function($model) {
                   if ($model->assign_date[0] == 0)
                      ...
                      return substr($model->assign_date, 0, -3);
                },
                'contentOptions' => ['class' => 'cassign_date'],
            ],
            [
                'attribute' => 'comment',
                'content' => function($model) {
        ...
        <div class="container-fluid">
           ...
           <div class="row">
              <div class="col-sm-12">
                 <ul class="nav nav-tabs">
                    <li role="presentation" class="active"><a href="<?= Url::toRoute('site/sims') ?>">Sims</a></li>
           ...
           <div class="row">
              <div class="col-sm-7">
                 <div class="input-group">
                    <textarea id="magic-numbers" class="form-control" rows="3" placeholder="Please choose sims and click 'Select' OR CTRL+ENTER. Examples: '12000001-12004000,1200*000, 12000001, 12000002, 12000001' OR just paste a column from EXCEL"></textarea>           
                 ...
                 <div id="magic-search-fields">
                    <label class="radio-inline">
                       <input type="radio" name="magic-field" id="inlineRadio1" value="iccid" checked="checked"> by ICCID
                    </label>
                    <label class="radio-inline">
                       <input type="radio" name="magic-field" id="inlineRadio2" value="msisdn"> by MSISDN
                    </label>
                    <label class="radio-inline">
                       <input type="radio" name="magic-field" id="inlineRadio3" value="dealer_id"> by Dealer nr.
                    </label>
                 </div>
              ...
              <div class="col-sm-5">
                 <div class="input-group">
                    ...
                    <input onClick="this.setSelectionRange(0, this.value.length)" value="" autocomplete="off" id="fast-dealer" type="text" class="form-control typeahead" placeholder="Enter dealer id or name" aria-describedby="basic-addon1">
                 </div>
           ...
           <script>
              ...
              function simEdit(d) {
                ...
                $('#modal').modal('show')
                        .find('#modalContent')
                        .load('<?= Url::to(['site/simedit']); ?>' + '?id=' + d.id);

              }
              function simSubmit($form) {
                $.post(
              ...
              function ready() {
                ...
                /*$('.field-sim-assign_date').datepicker({
                 format: "yyyy/mm/dd",
                 weekStart: 1
                 });*/
                ...
                $('#excel-btn').on('click', function (e) {
                  $('<form action="<?= Url::to(['site/getxlsx']); ?>" method="post" target="_blank"><input type="hidden" name="magic" value="' + magicData.magic + '"><input type="hidden" name="field" value="' + magicData.field + '"><input type="hidden" name="table" value="sim"></form>').appendTo('body').submit();
        });
